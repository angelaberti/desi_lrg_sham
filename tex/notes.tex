...
\documentclass[10pt, oneside]{}   	% use "amsart" instead of "article" for AMSLaTeX format
...
\citet{zu20} speculate that the low g-g lensing signal could be caused by BOSS CMASS galaxies residing in lower concentration halos. \citet{yuan_etal20, yuan_etal21} test concentration-based assembly bias models; summarized below), and/or not accounting for the dependence of the BOSS detection fraction at fixed stellar mass on halo properties that correlated with galaxy colors. This assessment is in qualitative agreement with the tests of assembly bias done by \citet{lange_etal19, lange_etal21a} and \citet{yuan_etal20, yuan_etal21} described below.
...
\end{enumerate}
% Find clustering signal difference of 10\% at $0.6 < z < 0.74$ and up to 150\% at $z=0.93$

\section*{Motivation for SHAM + age matching modeling of DESI LRGs}

\begin{itemize}
\item Smaller volume simulations (Bolshoi-Planck) with mass resolution sufficient for subhalo detection and tracking across snapshots enables small-scale clustering analysis that can't be done with large-volume mocks for BAO studies.
...
\item Empirical forward modeling provides a way to quantify incompleteness, satellite fraction, etc.\ of LRG sample using the full galaxy population (DESI Legacy Survey photometry).
...
\small
\bibliography{/Users/aberti/Desktop/Research/refs}
...
%abundance matching luminosity with halo proxy
